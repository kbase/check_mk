...
print(str(collectorState) + ' Condor_num_collectors collectors=' + str(numCollectors) + ' ' + collectorStateText + ' - ' + str(
    numCollectors) + ' collectors running')
...
print(str(negotiatorState) + ' Condor_num_negotiators negotiators=' + str(numNegotiators) + ' ' + negotiatorStateText + ' - ' + str(
    numNegotiators) + ' negotiators running')
...
for slot in slots:
    ...
    if not is_healthy:
        # Check for child activity
        child_activity = slot.get('ChildActivity', {})
        if not child_activity:
            # Mark as CRITICAL only if no active child activity
            slotState = 2
            slotStateText = 'CRITICAL'


    print(str(slotState) + ' Condor_slot_' + slot['Name'] + ' state=' + str(slot['Activity']) + ' ' + slotStateText + ' - slot ' + slot[
        'Name'] + ' in clientgroup ' + slot['CLIENTGROUP'] + ' is in state ' + slot['Activity'] + ' ' + node_is_healthy_message)
...
for clientgroup in conf.sections():
    ...
    try:
        ...
        print("%d Condor_clientgroup_%s %s=%d;%d;%d;0 %s - clientgroup %s has %d total workers and %d idle workers" % (
        clientgroupState, clientgroup, clientgroup, slotCounts[clientgroup]['Idle'], conf.getint(clientgroup, 'minIdle.warn'),
        conf.getint(clientgroup, 'minIdle.crit'), clientgroupStateText, clientgroup, slotCounts[clientgroup]['Total'],
        slotCounts[clientgroup]['Idle']))
...
print("%d Condor_idleCount idleCount=%d;%d;%d;0 %s - idleCount %d jobs idle" % (
idleCountState, idleJobCount, conf.getint('global', 'idlecount.warn'), conf.getint('global', 'idlecount.crit'), idleCountStateText, idleJobCount))
print("%d Condor_runningCount runningCount=%d;%d;%d;0 %s - runningCount %d jobs running" % (
runningCountState, runningJobCount, conf.getint('global', 'runcount.warn'), conf.getint('global', 'runcount.crit'), runningCountStateText,
runningJobCount))

print("%d Condor_idleTime idleTime=%d;%d;%d;0 %s - idleTime max %d minutes, sample 10 jobIds (minutes): %s" % (
idleTimeState, maxIdleTime, conf.getint('global', 'idletime.warn'), conf.getint('global', 'idletime.crit'), idleTimeStateText, maxIdleTime,
longIdleJobsText))
print("%d Condor_runningTime runningTime=%d;%d;%d;0 %s - runningTime max %d minutes, sample 10 jobIds (minutes): %s" % (
runningTimeState, maxRunningTime, conf.getint('global', 'runtime.warn'), conf.getint('global', 'runtime.crit'), runningTimeStateText, maxRunningTime,
longRunningJobsText))

print("%d Condor_expiredTokens - %s - %d jobs with expired tokens: %s" % (
expiredTokenState, expiredTokenStateText, len(expiredTokenJobsList), expiredTokenJobsText))
